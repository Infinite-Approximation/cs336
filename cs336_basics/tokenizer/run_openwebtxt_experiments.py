...
def run_experiment():
    # 1. 路径和参数配置
    # 注意：请根据你的实际路径修改数据集位置
    input_path = "/home/jkd/online_course_learning/cs336/assignment1-basics/data/owt_valid.txt"
    vocab_size = 32000
    ...
    with open("cs336_basics/res/openwebtxt_stats.json", "w") as f:
        json.dump(stats, f, indent=4)
    print(f"性能指标已保存至: cs336_basics/res/openwebtxt_stats.json")
    ...
    with open("cs336_basics/res/openwebtxt_vocab.json", "w") as f:
        json.dump(serializable_vocab, f)

    # 保存合并规则 (存为文本，每行一对)
    # 我们用十六进制表示字节，防止特殊字符导致文件不可读
    with open("cs336_basics/res/openwebtxt_merges.txt", "w") as f:
        for m1, m2 in merges:
            f.write(f"{m1.hex()} {m2.hex()}\n")

    print("保存成功：openwebtxt_vocab.json, openwebtxt_merges.txt")
